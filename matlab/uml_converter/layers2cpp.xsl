...
<!-- Use only saxon6.5.5 compatible constructs here. See http://saxon.sourceforge.net/saxon6.5.5/ -->

<!-- =================================================================== -->
<!-- Rendering starts here -->
<xsl:template match="/">#pragma once

//WARNING: This file was autogenerated. All changes will be lost without notice should the file be re-generated again!

#include &quot;_xml_generated_base_<xsl:value-of select="$generatedBaseVersion"/>.h&quot;
// Note: suitable version of _xml_generated_base_<xsl:value-of select="$generatedBaseVersion"/>.h file is described below after the struct <xsl:value-of select="/class/@name"/> definition.
// Copy it to a separate file.

// Also note, that other necessary header files (such as nntl definitions) are not included here.
// They are expected to be included before inclusion of this header.
// Also the file should be included inside namespace nntl{}, or using namespace nntl; should be defined before inclusion
...
struct <xsl:value-of select="/class/@name"/> : public _xml_generated_base_<xsl:value-of select="$generatedBaseVersion"/>&lt;typename P::real_t, typename P::PL<xsl:value-of select="/class/@name"/>&gt; {	
	//////////////////////////////////////////////////////////////////////////
	// Types aliases/declarations
	typedef typename P::PLId PLId;
	<xsl:if test='/class/types'>
		<xsl:text>
	// custom or enhanced standard classes declarations
	</xsl:text>
...
<xsl:if test="$istypedef=0 and @repeat"><xsl:call-template name="repeatWrapperTypeBegin" /></xsl:if>
...
<xsl:if test="$istypedef=0 and @repeat"><xsl:call-template name="repeatWrapperTypeEnd"/></xsl:if>
</xsl:template>

<!-- =================================================================== -->
<!-- Generates typename of wrapper that implementing repeat property-->
<xsl:template name='repeatWrapperTypeBegin'>
<xsl:text>::nntl::hlpr_array_of_layers_same_phl&lt;</xsl:text>
</xsl:template>

<xsl:template name='repeatWrapperTypeEnd'>
<xsl:text>&gt;</xsl:text>
</xsl:template>

<!-- =================================================================== -->
<!-- Generates layer object name (we make special name for .repeat layer objects to catch their misuse easier) -->
<xsl:template name='makeLayerObjName'>
<xsl:param name='nameBaseLayer'>0</xsl:param>
<xsl:param name='nodeName'><xsl:value-of select='@name'/></xsl:param>
<xsl:param name='hasRepeat'><xsl:value-of select='count(@repeat)'/></xsl:param>

<xsl:if test="$hasRepeat&gt;=1 and $nameBaseLayer=0"><xsl:text>arrl</xsl:text></xsl:if>
<xsl:if test="not($hasRepeat&gt;=1 and $nameBaseLayer=0)"><xsl:text>l</xsl:text></xsl:if>
...
<xsl:for-each select='/class/layers//*[@name and @id]'>
	...
	<xsl:variable name="curLayerName"><xsl:call-template name="makeLayerObjName"/></xsl:variable>
	<xsl:variable name="curBaseLayerName">
		<xsl:call-template name="makeLayerObjName"><xsl:with-param name='nameBaseLayer' select='1'/></xsl:call-template>
	</xsl:variable>	
	...
	<xsl:for-each select='/class/loss_addendums//*[@ref=$curId]/..'>
		<!-- <xsl:text></xsl:text><xsl:value-of select='$curLayerName'/>.addendum&lt;<xsl:call-template name="makeLossAddendumType"
			/>&gt;().scale(PL::<xsl:value-of select='$curBaseLayerName'/>_<xsl:value-of select='name()'/><xsl:text>Scale);
		</xsl:text> -->
		<xsl:text></xsl:text><xsl:value-of select='$curLayerName'/><xsl:text>.apply([](auto&amp; lyr)noexcept{
			lyr.addendum&lt;</xsl:text><xsl:call-template name="makeLossAddendumType"
			/>&gt;().scale(PL::<xsl:value-of select='$curBaseLayerName'/>_<xsl:value-of select='name()'/><xsl:text>Scale);
		});
		</xsl:text>		
	</xsl:for-each>
...
<xsl:template match='lph' mode='construct'>
	...
	<!-- constructing inner layers first -->
	...
	<!-- constructing lph object. Regarding .repeat layers see comments to mode='describe' template -->
	<xsl:text>
		, </xsl:text><xsl:call-template name="makeLayerObjName"/>(<xsl:call-template name="makeLayerName"/><xsl:text>, </xsl:text>
			<!-- choosing proper rendering branch with or without .repeat'ed layers -->
		<xsl:variable name="numRepeats" select='count(./phl/*[@name and @repeat])'/>
		<xsl:variable name="numTotal" select='count(./phl/*[@name])'/>
		<xsl:choose>
			<xsl:when test='$numRepeats &gt; 1'>
				<xsl:message terminate="yes">We don't support more than one .repeat'ed layer, check LPH <xsl:value-of select='name()'/> children</xsl:message>
			</xsl:when>
			<xsl:when test='$numRepeats=1 and $numTotal=1'>
				<!-- calling .to_phl<RepN>() to obtain tuple of PHL's of .repeat'ed layers -->
				<xsl:for-each select="./phl">
					...
					<xsl:call-template name="makeLayerObjName"><xsl:with-param name='nodeName' select='./*[@name]/@name'/>
					<xsl:with-param name='hasRepeat' select='count(./*[@name]/@repeat)'/></xsl:call-template>
					<xsl:text>.to_phl&lt;</xsl:text><xsl:value-of select='./*[@name]/@repeat'/><xsl:text>&gt;(</xsl:text>
					
					<!-- pluging offset and count parameters -->
					<xsl:if test='./ofs'>
			...
			<xsl:otherwise>
				<!-- No .repeat`ed layers inside, so just implement PHLs in usual way -->
				<xsl:text>::std::make_tuple(
			</xsl:text>
				<xsl:for-each select="./phl">
					<xsl:call-template name="beautifyEnumeration"><xsl:with-param name='separator'>,</xsl:with-param>
						<xsl:with-param name='breakline' select='1'/><xsl:with-param name='addspacing'><xsl:text>	</xsl:text></xsl:with-param>
					</xsl:call-template>
					<xsl:text>make_PHL(</xsl:text>
						<xsl:call-template name="makeLayerObjName"><xsl:with-param name='nodeName' select='./*[@name]/@name'/></xsl:call-template>
	...
	<xsl:text>)</xsl:text><!-- finalizing call to constructor -->
</xsl:template>
...
<xsl:template match='lpv' mode='construct'>
	...
	<xsl:text>
		, </xsl:text><xsl:call-template name="makeLayerObjName"/>(<xsl:call-template name="makeLayerName"/><xsl:text>, ::std::tie(</xsl:text>
	<xsl:for-each select="./*[@name]">
		...
		<xsl:call-template name="makeLayerObjName"/><xsl:text></xsl:text>
...
<xsl:template match='lid' mode='construct'>
	...
	<xsl:call-template name="makeLayerObjName"/>(<xsl:call-template name="makeLayerName"/><xsl:text>)</xsl:text>
</xsl:template>
...
<xsl:template match='lfc|custom' mode='construct'>
	...
	<xsl:if test='not(name()="custom" and @constr)'>
		<xsl:text></xsl:text>
		<xsl:if test='@repeat'><xsl:value-of select='@repeat'/><xsl:text>, </xsl:text></xsl:if>
		<xsl:call-template name="makeLayerName"/>, nc_<xsl:value-of select='@name'/>, learningRate<xsl:text></xsl:text>
	</xsl:if>
...
<!-- =================================================================== -->
<!-- declaring layer objects during "describe" phase consists of two steps: -->
<!-- describing neurons count of a layer and declaring layer object with corresponding type -->
<xsl:template match='lph' mode='describe'>
	...
	<xsl:for-each select="./phl/*[@name]">
		...
		<xsl:if test='@repeat'> <!-- we need special handling for .repeat'ed layers -->
	...
	<!-- Now we'd descrive the layer type. It's a real burden to mix .repeat with no.repeat layers, so for now just -->
	<!-- allow a single one .repeat layer only or any mix of no .repeat layers-->
	<xsl:call-template name="makeLayerType"/><xsl:text>&lt;</xsl:text>
	...
	<xsl:choose>
		...
		<xsl:when test='$numRepeats=1 and $numTotal=1'>
			<!-- PHL's of .repeat'ed layers could be described with type of .to_phl<RepN>() return value (which is correct -->
			<!-- tuple to pass to LPHt constructor) -->
			<xsl:for-each select="./phl/*[@name and @repeat]">
				<!-- <xsl:call-template name="beautifyEnumeration"><xsl:with-param name='separator'>,</xsl:with-param>
					<xsl:with-param name='breakline' select='3'/></xsl:call-template> -->
				<xsl:text>decltype(</xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text>.to_phl&lt;</xsl:text>
				<xsl:value-of select='@repeat'/>
				<xsl:text>&gt;(0,0)</xsl:text><!-- actual run-time .to_phl() parameters values are irrelevant here -->
				<xsl:text>)</xsl:text>
			</xsl:for-each>
		</xsl:when>
		<xsl:otherwise>
			<!-- No .repeat`ed layers inside, so just describe PHLs in usual way -->
			<xsl:text>::std::tuple&lt;</xsl:text>
			<xsl:for-each select="./phl/*[@name]">
				<xsl:call-template name="beautifyEnumeration"><xsl:with-param name='separator'>,</xsl:with-param>
					<xsl:with-param name='breakline' select='3'/></xsl:call-template>
				<xsl:text>::nntl::PHL&lt;decltype(</xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text></xsl:text>
				<xsl:if test='@constr and name()="custom" and @type'>
					<xsl:text>.lFinal</xsl:text>
				</xsl:if>
				<xsl:text>)&gt;</xsl:text>
	...
	<xsl:text>&gt; </xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text>;	
...
<xsl:template match='lpv' mode='describe'>
	...
	<xsl:for-each select="./*[@name]">
		...
		<xsl:text>decltype(</xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text></xsl:text>
		<xsl:if test='@constr and name()="custom" and @type'>
			<xsl:text>.lFinal</xsl:text>
		</xsl:if>
		<xsl:text>)&amp;</xsl:text>
...
<xsl:template match='lid' mode='describe'>
	...
	<xsl:text>;	
	</xsl:text><xsl:call-template name="makeLayerType"/><xsl:text> </xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text>;	
	</xsl:text>
</xsl:template>
...
<xsl:template match='custom' mode='describe'>
	...
	<xsl:call-template name="makeLayerType"/><xsl:text> </xsl:text><xsl:call-template name="makeLayerObjName"/><xsl:text>;	
	</xsl:text>
</xsl:template>
...
<xsl:template name='describeSrc'>
	<xsl:text>
	//////////////////////////////////////////////////////////////////////////
	// Source data (receptive field) description:
	</xsl:text>
...
<xsl:template name='makeLayerName'>
	<xsl:text>"</xsl:text><xsl:value-of select="/class/@name"/>__<xsl:value-of select="@name"/><xsl:text>"</xsl:text>
	<!-- <xsl:call-template name="makeLayerObjName">
		<xsl:with-param name='nameBaseLayer' select='1'/></xsl:call-template> -->
</xsl:template>
...
// (extract it to the separate file and place it nearby)
//////////////////////////////////////////////////////////////////////////
/*
#pragma once
...
public:	
	...
	//ncMultiplier will be used when neurons count was set to numeric value in uml model
