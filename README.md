...
* The following feedforward layer types has been implemented allowing one to create many types of feedforward neuron connectivity:
  * ordinary layers:
   * *layer_fully_connected* is the basic layer type where all the magic happens
   * *layer_output is a variation* of fully connected layer spcialized to be an output of a neural network
   * *layer_input* provides a common interface to a data for hidden layers
   * *layer_identity* allows to pass an incoming data to upper layers (used inside of compound layers)
   * *layer_identity_gate* passes incoming neurons up to a layer stack while allowing *_gated layer types to use them as gating values.
  * compound layers (this layer types allow to encapsulate other layers in some way to produce more sophisticated architectures):
   * *layer_pack_gated* helps to deal with optional source data and allows to train a feature detector that is specialized only on that optional part.
   * *layer_pack_horizontal* is designed to feed different sequential (possibly overlapping) ranges of underlying neurons to a corresponding different layers. For example, this allows one to build a set of feature detectors each of which is specialized on a specific subset of data features.
